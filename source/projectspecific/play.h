void play(float gamespeed){
	...
	if(keystate[SDL_SCANCODE_A]){
		Mix_HaltMusic();
		Mix_PlayChannel( -1, normal, 0 );
		status=0;
	}

	if(keystate[SDL_SCANCODE_1])mode=1;
	if(keystate[SDL_SCANCODE_2])mode=2;
	if(keystate[SDL_SCANCODE_3])mode=3;
	if(keystate[SDL_SCANCODE_4])mode=4;


	if(keystate[SDL_SCANCODE_X])camz+=0.1;
	if(keystate[SDL_SCANCODE_Z])camz-=0.1;
	...
	//	bool run=0;
		if(!a){
			...
			if(keystate[SDL_SCANCODE_LEFT])left=1;
			if(keystate[SDL_SCANCODE_RIGHT])right=1;
		...
		//	if(keystate[SDLK_LSHIFT])run=1;
			if(keystate[SDL_SCANCODE_SPACE]==2|keystate[SDL_SCANCODE_RETURN]==2)pause=1;
		}
		/*
	...
	*/
	...
	//spawn zombies in the building
	if(persontypecount[persontype_zombie]<400 && (persontypecount[persontype_zombie]+persontypecount[persontype_survivor])<500){
		float x=(mapsizex/2)+4-(float)(rand()%9);
		float y=(mapsizey/2)+4-(float)(rand()%9);
		if(touchworld(x,y,0.23f,1)==0)
			makeperson(persontype_zombie,x,y,0.f,0.f,randangle());
	}
	...
	for(int a=0;a<personcount;a++){
		...
		if(personlist[a].type==persontype_player){
			// Persons update, empty for now
		}
		//update the aware people
			 ...
			 if(personlist[a].updateturn==updateturn || personlist[a].havetarget==0){
				...
				float distance=findnearesttarget(a,persontype_player,30.f);
				...
				if(status){	
					...
					case 1:
						personlist[a].angle=atan2(
								((mapsizey+11)/2)+3-personlist[a].posy,
								(mapsizex/2)-0.5f-personlist[a].posx);
						personlist[a].movx+=cos(personlist[a].angle)*zombiemovespeed*(1+((rand()%2)/3));
						personlist[a].movy+=sin(personlist[a].angle)*zombiemovespeed*(1+((rand()%2)/3));
						break;
					case 2:
						if(personlist[a].posy<(((mapsizey+11)/2)-1)){
							personlist[a].angle=atan2(
								((mapsizey+11)/2)-1-personlist[a].posy,
								(mapsizex/2)-0.5f-personlist[a].posx);
							personlist[a].movx+=cos(personlist[a].angle)*zombiemovespeed*(1+((rand()%3)/3));
							personlist[a].movy+=sin(personlist[a].angle)*zombiemovespeed*(1+((rand()%3)/3));
						}
						else{
						personlist[a].angle=atan2(
								((mapsizey+11)/2)+3-personlist[a].posy,
					...
					case 3:
						if((rand()%10)<1)
							personlist[a].angle=atan2(
								((mapsizey+11)/2)+3-personlist[a].posy,
								(mapsizex/2)-0.5f-personlist[a].posx)-0.5f+((rand()%10)/10.f);
						personlist[a].movx+=cos(personlist[a].angle)*zombiemovespeed*(1+((rand()%2)/3));
						personlist[a].movy+=sin(personlist[a].angle)*zombiemovespeed*(1+((rand()%2)/3));
						break;
					}
					...
					if((rand()%100)<1)
						personlist[a].angle=PI*(rand()%36)/18;
					personlist[a].movx+=cos(personlist[a].angle)*zombiemovespeed*(4-(rand()%10));
					personlist[a].movy+=sin(personlist[a].angle)*zombiemovespeed*(4-(rand()%10));
				}
			}
		...
		*/
		...
		if(removed){
			...
			&& personlist[a].posy<personlist[b].posy+0.46f && personlist[a].posy>personlist[b].posy-0.46f){
				float distance=dist2d(personlist[a].posx,personlist[a].posy,personlist[b].posx,personlist[b].posy);
				if(distance<0.46f){
					/*
					...
					*/
					...
					float angle=atan2(personlist[a].posy-personlist[b].posy,personlist[a].posx-personlist[b].posx);
