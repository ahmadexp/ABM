...
void genworldgrid(){
	...
	//clean the map
	for(int x=0;x<mapsizex;x++)for(int y=0;y<mapsizey;y++)map[x][y]=tiletype_grass;
	for(int x=0;x<mapsizex;x++)for(int y=0;y<mapsizey;y++)maprotation[x][y]=0;

	const int roomx=11;
	const int roomy=11;

	for(int x=(mapsizex-roomx)/2; x<(mapsizex+roomx)/2; x++)
		for(int y=(mapsizey-roomy)/2; y<(mapsizey+roomy)/2; y++)
			map[x][y]=tiletype_street;

	for(int x=((mapsizex-roomx)/2)-1; x<((mapsizex+roomx)/2)+1; x++){
		map[x][(int)((mapsizey-roomy)/2)-1]=tiletype_sidewalk;
		map[x][(int)((mapsizey+roomy)/2)]=tiletype_sidewalk;
	}
	for(int y=((mapsizey-roomy)/2)-1; y<((mapsizey+roomy)/2)+1; y++){
		map[(int)((mapsizey-roomy)/2)-1][y]=tiletype_sidewalk;
		map[(int)((mapsizey+roomy)/2)][y]=tiletype_sidewalk;
	}

	map[(int)((mapsizey-roomy)/2)-1][(int)((mapsizey+roomy)/2)]=tiletype_sidewalkcorner;
	maprotation[(int)((mapsizey-roomy)/2)-1][(int)((mapsizey+roomy)/2)]=0;
	map[(int)((mapsizey-roomy)/2)-1][(int)((mapsizey-roomy)/2)-1]=tiletype_sidewalkcorner;
	maprotation[(int)((mapsizey-roomy)/2)-1][(int)((mapsizey-roomy)/2)-1]=1;
	map[(int)((mapsizey+roomy)/2)][(int)((mapsizey-roomy)/2)-1]=tiletype_sidewalkcorner;
	maprotation[(int)((mapsizey+roomy)/2)][(int)((mapsizey-roomy)/2)-1]=2;
	map[(int)((mapsizey+roomy)/2)][(int)((mapsizey+roomy)/2)]=tiletype_sidewalkcorner;
	maprotation[(int)((mapsizey+roomy)/2)][(int)((mapsizey+roomy)/2)]=3;

	for(int x=((mapsizex-roomx)/2)+1; x<((mapsizex+roomx)/2)-1; x++){
		map[x][(int)((mapsizey-roomy)/2)+1]=tiletype_buildingwall;
		maprotation[x][(int)((mapsizey-roomy)/2)+1]=1;
		map[x][(int)((mapsizey+roomy)/2)-2]=tiletype_buildingwall;
		maprotation[x][(int)((mapsizey+roomy)/2)-2]=3;
	}
	for(int y=((mapsizey-roomy)/2)+1; y<((mapsizey+roomy)/2)-1; y++){
		map[(int)((mapsizey-roomy)/2)+1][y]=tiletype_buildingwall;
		maprotation[(int)((mapsizey-roomy)/2)+1][y]=0;
		map[(int)((mapsizey+roomy)/2)-2][y]=tiletype_buildingwall;
		maprotation[(int)((mapsizey+roomy)/2)-2][y]=2;
	}

	map[(int)((mapsizey-roomy)/2)+1][(int)((mapsizey+roomy)/2)-2]=tiletype_buildingcorner;
	maprotation[(int)((mapsizey-roomy)/2)+1][(int)((mapsizey+roomy)/2)-2]=0;
	map[(int)((mapsizey-roomy)/2)+1][(int)((mapsizey-roomy)/2)+1]=tiletype_buildingcorner;
	maprotation[(int)((mapsizey-roomy)/2)+1][(int)((mapsizey-roomy)/2)+1]=1;
	map[(int)((mapsizey+roomy)/2)-2][(int)((mapsizey-roomy)/2)+1]=tiletype_buildingcorner;
	maprotation[(int)((mapsizey+roomy)/2)-2][(int)((mapsizey-roomy)/2)+1]=2;
	map[(int)((mapsizey+roomy)/2)-2][(int)((mapsizey+roomy)/2)-2]=tiletype_buildingcorner;
	maprotation[(int)((mapsizey+roomy)/2)-2][(int)((mapsizey+roomy)/2)-2]=3;

	for(int x=((mapsizex-roomx)/2)+2; x<((mapsizex+roomx)/2)-2; x++)
		for(int y=((mapsizey-roomy)/2)+2; y<((mapsizey+roomy)/2)-2; y++)
			map[x][y]=tiletype_buildinginside;

	map[(int)(mapsizex/2)-1][(int)((mapsizey+roomy)/2)-2]=tiletype_buildingdoor;
	maprotation[(int)((mapsizey/2)-1)][(int)((mapsizey+roomy)/2)-2]=3;
	...
	*/
	...
	for(int x=0; x<smallmapsizex; x++)
		for(int y=0; y<smallmapsizey; y++)
			if(watergrid[x][y]==0)
