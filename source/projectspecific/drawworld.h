void drawworld(){
	...
	set2dcamera(camx-camz*0.5f*screena, camx+camz*0.5f*screena, camy-camz*0.5f, camy+camz*0.5f);
	...
	for(int x=lowx;x<highx;x++)for(int y=lowy;y<highy;y++)
		draw2dquad((float)x+0.5f,(float)y+0.5f,1.f,1.f,tilesettexturecoords[(int)map[x][y]],
			1,(float)maprotation[x][y]*90.f*radiansindegree);

	//draw decals
	for(int a=0;a<decalcount;a++){
		glColor4f(1.f,1.f,1.f,(float)a/(float)maxdecalcount); 
		draw2dquad(decallist[a].posx,decallist[a].posy,0.75f,0.75f,decaltexturecoords[decallist[a].type],
			1,decallist[a].angle);
	}
	glColor4f(1.f,1.f,1.f,1.f); 
	
	//draw reticle
	//for(int a=0;a<playercount;a++)draw2dquad(aiminworldx[a],aiminworldy[a],1.5f,1.5f,reticletexturecoords,1,0.f);
	...
	for(int a=0;a<explosioncount;a++){
		...
		draw2dquad(explosionlist[a].posx,explosionlist[a].posy,2.75f,2.75f,explosiontexturecoords,1,explosionlist[a].angle);
	}
	*/
	...
	*/
	...
	for(int x=lowx;x<highx;x++)for(int y=lowy;y<highy;y++)for(int a=0;a<wallcount[x][y];a++)
		draw2dquad(
			walllist[x][y][a].posx,walllist[x][y][a].posy,walllist[x][y][a].sizex,walllist[x][y][a].sizey,
			walltexturecoords,0,0.f);

	//draw me
	draw2dquad((mapsizex/2)-.5,(mapsizey/2)+8,1.f,1.f,cratetexturecoords[2],1,0);

	//draw cloud shadows
	glBindTexture(GL_TEXTURE_2D,cloudshadowtexture);
	
	for(int x=lowx;x<highx+10;x+=10)for(int y=lowy;y<highy+10;y+=10)
		draw2dquad(floor((float)x/10.f)*10.f+0.5f,floor((float)y/10.f)*10.f+0.5f,10.f,10.f,cloudshadowtexturecoords,1,0.f);
