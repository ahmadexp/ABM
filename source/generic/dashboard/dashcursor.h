...
void setupdashcursor(){
	...
	for(int a=0; a<5; a++){
		dashgentexturecoords(dash_cursor_texturecoords[a],
			440+a*40,60,40,60);
	}
...
void dashdrawcursor(int x, int y,int cursornum){
	...
	dashprepdraw();
	draw2dquad(x-12,y-10,40,60,dash_cursor_texturecoords[cursornum]);
	dashdonedraw();
...
void dashcursors(){
	...
	if(window && (SDL_GetWindowFlags(window) & SDL_WINDOW_MOUSE_FOCUS)) dashusecursor[0]=1;
	if(dashusecursors){
